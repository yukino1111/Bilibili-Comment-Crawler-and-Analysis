...
class GetInfo:
    def __init__(self, user_id):
        ...
        self.d = webdriver.Chrome(options=chrome_options)
    ...
    def get_url(self):
        ...
        try:
            ...
            #     # 将JSON字符串写入文件
            #     f.write(data)
            # print(f"已获取当前页面视频aid，总数: {len(self.a_list)}")

        except Exception as e:
            print(f"获取当前页面视频aid失败: {e}")
...
if __name__ == "__main__":
    ...
    user_id_to_crawl = "163637592"
    crawler = GetInfo(user_id_to_crawl)
    video_aids = crawler.next_page()
    print(f"最终获取到的视频aid数量: {len(video_aids)}")
