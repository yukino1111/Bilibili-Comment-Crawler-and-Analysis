...
from utils.get_csv import export_comments_by_oid_to_csv
...
if __name__ == "__main__":
    ...
    try:
        # 重新获取 BV1fW411E7Vd 的 oid，确保查询准确
        temp_crawler = BilibiliCommentCrawler(bv=bvs)
        video_oid, title = temp_crawler.get_information()  # 这可能会再次发起网络请求
        comments_from_db = comment_repo.get_comments_by_oid_paginated(
        ...
        for user in users_from_db:
            print(f"  - Mid: {user.mid}, Name: {user.name}, Sex: {user.sex}")
